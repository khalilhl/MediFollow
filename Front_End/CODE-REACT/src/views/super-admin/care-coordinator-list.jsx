  ...
  return (
    <>
          ...
          <Card>
            <Card.Header className="d-flex justify-content-between align-items-center flex-wrap gap-2">
              <div className="header-title">
                <h4 className="card-title mb-0">Liste des Care Coordinators</h4>
                <p className="text-muted mb-0 small">{filtered.length} care coordinator(s)</p>
              </div>
              <Link to="/super-admin/care-coordinators/add" className="btn btn-primary btn-sm">
                <i className="ri-add-line me-1"></i>Ajouter un Care Coordinator
              </Link>
            </Card.Header>
            <Card.Body>
              ...
              <Row className="g-3">
                {filtered.length === 0 ? (
                  <Col sm={12}><div className="text-center text-muted py-5">Aucun care coordinator trouvé</div></Col>
                ) : filtered.map((c) => (
                  <Col key={c.id} xl={3} md={4} sm={6}>
                    <Card className="border shadow-sm h-100">
                      <Card.Body className="text-center p-3">
                        <img src={getImageSrc(c)} alt="" className="rounded-circle mb-2 border"
                          style={{ width: 72, height: 72, objectFit: "cover" }} />
                        <h6 className="mb-0">{c.name || `${c.firstName || ""} ${c.lastName || ""}`.trim() || "—"}</h6>
                        <small className="text-muted">{c.email}</small>
                        <div className="mt-2">
                          <span className="badge bg-info bg-opacity-10 text-info">{c.specialty || "Care Coordinator"}</span>
                        </div>
                        <div className="mt-2">
                        ...
                        <div className="d-flex justify-content-center gap-2 mt-3">
                          <Link to={`/super-admin/care-coordinators/${c.id}`} className="btn btn-sm btn-primary-subtle" title="Voir">
                            <i className="ri-eye-line"></i>
                          </Link>
                          <Link to={`/super-admin/care-coordinators/edit/${c.id}`} className="btn btn-sm btn-info-subtle" title="Modifier">
                            <i className="ri-edit-line"></i>
                          </Link>
                          <button className="btn btn-sm btn-danger-subtle" onClick={() => setToDelete(c)} title="Supprimer">
      ...
      <ConfirmActionModal
        ...
        title="Supprimer le Care Coordinator"
